...
const FileSpreadsheetIcon = (props: any) => (
  <svg {...props} fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const LoadingIcon = (props: any) => (
  <svg className={`animate-spin ${props.className}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
  </svg>
);
...
export const EventLogComponent: React.FC<EventLogComponentProps> = ({
  className = '',
  maxHeight = '600px',
  showRealTime = true,
  initialFilter = {}
  ...
  // State
  const [events, setEvents] = React.useState<DecodedEvent[]>([]);
  const [filteredEvents, setFilteredEvents] = React.useState<DecodedEvent[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [realTimeEnabled, setRealTimeEnabled] = React.useState(showRealTime);
  const [showFilters, setShowFilters] = React.useState(false);
  const [hasMore, setHasMore] = React.useState(true);
  const [loadingMore, setLoadingMore] = React.useState(false);
  
  // Filter state
  const [filters, setFilters] = React.useState<EventFilter>(initialFilter);
  ...
  // Load initial events
  const loadEvents = React.useCallback(async (reset = false) => {
  ...
  const applyFilters = React.useCallback((events: DecodedEvent[], filters: EventFilter, search: string) => {
    let filtered = events;
  ...
  const setupWebSocketSubscription = React.useCallback(() => {
...
const EventFiltersComponent: React.FC<EventFiltersComponentProps> = ({ filters, onFilterChange, eventDecoder }) => {
